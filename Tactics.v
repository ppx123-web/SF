...
Proof.
    ...
    - intros m eq. destruct m as [|m'].
        ...
        -- simpl in eq. apply f_equal. apply IHn'. apply eq.
Qed.
...
Proof.
    ...
    - intros m eq. destruct m as [|m']. 
        -- discriminate eq.
        -- simpl in eq. apply f_equal. injection eq as eq1. rewrite <- plus_n_Sm in eq1. rewrite <- plus_n_Sm in eq1. injection eq1 as eq2. apply IHn'. apply eq2.
Qed.
(** [] *)
...
Proof.
    ...
    - intros. simpl. destruct n as [|n'].
        ...
        -- simpl. simpl in H. injection H as H1. apply IHl' in H1. apply H1.
Qed.
(** [] *)
...
Proof.
    ...
    induction l as [|h t IHl'].
    - intros. simpl in H. injection H as H1 H2.
        symmetry in H1. symmetry in H2. replace l1. replace l2. reflexivity.
    - intros. destruct h as (x,y). simpl in H. destruct (split t) as (lx, ly). injection H as H1 H2. rewrite <- H1. rewrite <- H2. simpl. apply f_equal. apply IHl'. reflexivity.
Qed.
(** [] *)
...
Proof.
    ...
    - intros. simpl in H. destruct m as [|].
        -- discriminate H.
        -- simpl in H0. destruct p as [|].
            --- discriminate H0.
            --- simpl. apply IHn' with m. apply H. apply H0.
Qed.
(** [] *)
...
Definition split_combine_statement : Prop
    ...
    := forall X Y l1 l2 (l : list (X * Y)), length l1 = length l2 ->
...
Proof.
    ...
    generalize dependent l1.
    induction l as [|h t IHl'].
    - intros. destruct l1 as [|].
        -- destruct l2 as [|].
            --- reflexivity.
            --- simpl in H. discriminate H.
        -- simpl. destruct l2 as [|].
            --- discriminate H.
    ...
    - intros. destruct l1 as [|].
        ...
        -- destruct l2 as [|].
            --- discriminate H.
            --- simpl in H. injection H as H1. simpl in H0. injection H0 as H2. simpl. replace h.
            assert(H3: split t = (l1,l2)). {
                apply IHl' in H1.
                apply H1.
                apply H.
            }
            replace (split t). reflexivity.
Qed.
...
Proof.
    ...
    - intros. simpl in H. destruct (test h) eqn:eq.
        -- injection H as eq1. rewrite eq1 in eq. apply eq.
...
Proof.
    ...
    - simpl. destruct (test h) eqn:eq1.
        -- unfold existsb'. unfold forallb. rewrite eq1. simpl.  reflexivity.
        -- unfold existsb'. unfold forallb. rewrite eq1. simpl. rewrite IHl'. reflexivity.
Qed.
