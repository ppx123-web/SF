...
Proof.
  intros.
  unfold not in H.
  apply ex_falso_quodlibet. apply H. apply H0.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Theorem double_neg_inf: forall (P : Prop), P -> ~~P.
Proof.
  unfold not.
  intros.
  apply H0. apply H.
...
Proof.
  ...
  - intros. destruct H as [|]. 
    -- split. 
      ...
      --- left. apply H.
    -- destruct H as []. split. 
      --- apply or_commut. apply or_intro_l. apply H.
      --- apply or_commut. apply or_intro_l. apply H0.
  - intros. destruct H as []. destruct H as [].
    -- apply or_intro_l. apply H.
    -- destruct H0 as [].
      --- apply or_intro_l. apply H0.
      --- right. apply and_intro. apply H. apply H0.
Qed.
(** [] *)
...
Proof.
  intros.
  split.
  - intros. destruct H as [x E]. destruct E as [].
    -- left. exists (x). apply H.
    -- right. exists (x). apply H.
  - intros. destruct H as [].
    -- destruct H as [x E]. exists (x). left. apply E.
    -- destruct H as [x E]. exists (x). right. apply E.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. simpl in H. destruct m as [|].
    -- discriminate H.
    -- apply IHn' in H. simpl. destruct H as [x E]. exists x. rewrite E. reflexivity.
Qed.
...
Proof.
  induction n as [|n' IHn'].
  - intros. reflexivity.
  - intros m [x H]. rewrite H. simpl. apply IHn'. exists x. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. induction l as [|h t IHl'].
    -- simpl in H.  simpl. destruct H as [].
    -- simpl in H. destruct H as [].
      --- exists h. split. apply H. simpl. left. reflexivity.
      --- simpl. apply IHl' in H. destruct H as [x E]. exists x. destruct E as []. split. apply H. right. apply H0.
  - intros. destruct H as [x E]. destruct E as []. rewrite <- H. apply In_map. apply H0.
Qed.
(** [] *)
...
Proof.
  ...
  - intros. simpl. split.
    ...
    -- intros. rewrite <- or_assoc in H. destruct H as [].
      --- left. apply H.
...
Theorem rev_append_correct : forall X (l1 l2 : list X),
  rev_append l1 l2 = rev l1 ++ l2.
Proof.
  intros X l1.
  induction l1 as [|h t IHl'].
  - simpl. reflexivity.
  - simpl. intros. rewrite IHl'. simpl.  rewrite <- app_assoc. reflexivity.
Qed.
...
Proof.
  ...
  intros l. unfold tr_rev. rewrite <- (app_nil_r _ (rev l)). apply rev_append_correct.
Qed.
(* Proof.
  intros.
  ...
  - destruct t eqn:eq1.
    -- reflexivity.
    -- simpl. simpl in IHl'.  rewrite  <- IHl'. unfold tr_rev. simpl. rewrite rev_append_correct. rewrite rev_append_correct. rewrite <- app_assoc. simpl. reflexivity.
Qed. *)
(** [] *)
...
Proof.
  ...
  - rewrite even_S. destruct (even n') eqn:eq1.
    -- simpl. destruct IHn' as [nx E]. exists nx. rewrite E. reflexivity.
    -- simpl. destruct IHn' as [nx E]. rewrite E. exists (S nx). simpl. reflexivity.
Qed.
...
Proof.
  ...
  - intros. destruct b1 as [].
    ...
    -- destruct b2 as [].
      --- simpl in H. discriminate H.
      --- simpl in H. discriminate H.
  - intros. destruct H as []. rewrite H. rewrite H0. reflexivity.
Qed.
...
Proof.
  intros.
  split.
  - intros. intros Hneq. rewrite <- not_true_iff_false in H. apply H. apply eqb_eq. apply Hneq.
  - intros. rewrite <- not_true_iff_false. intros Htrue. apply H. apply eqb_eq.  apply Htrue.
Qed.
(** [] *)
...
| h1 :: t1 => match l2 with
      | [] => false
      | h2 :: t2 => if (eqb h1 h2) then eqb_list eqb t1 t2 else false
      end
end
.
...
Proof.
  ...
  - intros. split.
    -- simpl. intros. destruct l2 as [|h2 t2].
      --- discriminate H0.
      --- destruct (eqb h1 h2) eqn:eq1.
        + apply IHl1' in H0. apply H in eq1. rewrite eq1. rewrite H0. reflexivity.
        + discriminate H0.
    -- intros. simpl.  destruct l2 as [|h2 t2].
      --- discriminate H0.
      --- destruct (eqb h1 h2) eqn:eq1.
        + apply IHl1'. injection H0 as H1 H2. apply H2.
        + injection H0 as H1 H2. apply H in H1. rewrite eq1 in H1. apply H1.
Qed.
...
Proof.
  ...
  - intros. induction l as [|h t IHl'].
    -- simpl. reflexivity.
    -- simpl. simpl in H. destruct (test h) eqn:eq1.
      --- split. reflexivity. apply IHl' in H. apply H.
      --- discriminate H.
  - intros. induction l as [|h t IHl'].
    -- simpl. reflexivity.
    -- simpl. simpl in H. destruct (test h) eqn:eq1.
      --- destruct H. apply IHl' in H0. apply H0.
      --- destruct H. apply H.
Qed.
...
Proof.
  ...
  - apply H1.
  - unfold not in H2. apply ex_falso_quodlibet. apply H0. exists x. apply H2.
Qed.
(** [] *)
...
Proof.
  unfold excluded_middle.  unfold peirce. intros. destruct (H P) as [].
  - apply H1.
  - apply H0. unfold not in H1. intros. apply H1 in H2. destruct H2.
Qed.
...
Proof.
  ...
  unfold peirce.
  intros.
  apply H with False. intros. unfold not in H0. destruct (H0 H1).
Qed.
...
Proof.
  ...
  apply H0.
  split.
  - unfold not. intros. apply H1. left. apply H2.
  - unfold not. intros. apply H1. right. apply H2.
Qed.
