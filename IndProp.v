...
Inductive clos_refl_tran {X: Type} (R: X->X->Prop) : X->X->Prop :=
| rt_refl (x: X) : clos_refl_tran R x x
| rt_step (x y : X) :
  R x y ->
  clos_refl_tran R x y
| rt_trans (x y z : X) :
  clos_refl_tran R x y ->
  clos_refl_tran R y z ->
  clos_refl_tran R x z.

Inductive clos_refl_symm_tran {X: Type} (R: X->X->Prop) : X->X->Prop :=
| rst_step (x y : X) :
  R x y ->
  clos_refl_symm_tran R x y
| rst_refl (x: X) : clos_refl_symm_tran R x x
| rst_symm (x y :X) : clos_refl_symm_tran R x y -> clos_refl_symm_tran R y x
| rst_trans (x y z : X) :
  clos_refl_symm_tran R x y ->
  clos_refl_symm_tran R y z ->
  clos_refl_symm_tran R x z.
...
Example Perm3Example1 : Perm3 [1;2;3] [3;2;1].
Proof.
...
Example Perm3Example2 : Perm3 [1;2;3] [1;2;3].
Proof.
  apply perm3_trans with [1;3;2].
  - apply perm3_swap23.
  - apply perm3_swap23.
...
Proof.
  intros.
  split.
  - intros. induction H as [ | |n' m' IH].
    -- apply ev_0.
    -- apply (ev_SS). apply ev_0.
    -- apply ev_sum. apply IHIH. apply IHev'1.
  - intros. induction H as [|n' E IH].
    -- apply ev'_0.
...
Proof.
  ...
  - simpl in H. apply H.
  - apply IH. simpl in H. inversion H. apply H1.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Theorem plus_double: forall n : nat, n + n = double n.
Proof.
  intros.
  induction n as [|n' IHn'].
  - simpl. reflexivity.
  - simpl. rewrite <- IHn'. apply f_equal. rewrite plus_n_Sm. reflexivity.
Qed.
...
Proof.
  ...
  - apply H1.
  - rewrite plus_double. apply ev_double.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
| total_relation_of n m : total_relation n m
.

Theorem total_relation_is_total : forall n m, total_relation n m.
Proof.
    intros.
    apply total_relation_of.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof.
  intros.
  induction H0 as [|o' E IH].
  - apply H.
  - apply le_S in IH. apply IH.
Qed.
...
Proof.
  ...
  - destruct IHn' as [|].
    -- inversion H as [|m'].
      + right. apply le_n.
      + left. unfold lt. apply n_le_m__Sn_le_Sm. apply H0.
...
Proof.
  ...
  - intros. simpl in H. split. apply O_le_n. apply H.
  - intros. destruct m as [].
    -- simpl in H. inversion H as [].
    -- simpl in H. apply Sn_le_Sm__n_le_m in H. apply IHn1' in H. destruct H.
    split. 
...
Proof.
  ...
  - intros. destruct p as [|p'].
    -- right. apply plus_le in H. destruct H. apply H0.
    -- simpl in H. rewrite plus_n_Sm in H. rewrite plus_n_Sm in H. apply IHn' in H.
    destruct H as [].
...
Proof.
  intros.
  induction p as [|p' IHp'].
  - rewrite <- plus_n_O. rewrite <- plus_n_O. apply H.
  - rewrite <- plus_n_Sm. rewrite <- plus_n_Sm. apply n_le_m__Sn_le_Sm. apply IHp'.
Qed.
  (* FILL IN HERE *)
...
Proof.
  ...
  - apply le_S in IHp'. rewrite plus_n_Sm in IHp'. apply IHp'.
Qed.
  (* FILL IN HERE *)
...
Proof.
  ...
  destruct m as [|].
  - inversion H.
  - apply Sn_le_Sm__n_le_m in H. apply plus_le in H. destruct H. split.
    -- apply n_le_m__Sn_le_Sm. apply H.
...
Proof.
  ...
  - intros. induction H as []. 
    ...
    -- simpl in IHR. injection IHR as E1. rewrite <- plus_n_Sm in E1. injection E1 as E2. apply E2.
    -- unfold fR. unfold fR in IHR. rewrite <- add_comm. apply IHR.
  - intros. generalize dependent n. generalize dependent o. induction m as [|m' IHm'].
    -- intros. simpl in H. rewrite H. 
      assert(H1: forall s, R 0 s s). {
...
| c1  : subseq [] []
| c2  x l1 l2 (H: subseq l1 l2): subseq l1 (x::l2)
| c3  x l1 l2 (H: subseq l1 l2): subseq (x::l1) (x::l2)
.
...
Proof.
  ...
  induction H as [].
  - apply subseq_empty.
  - simpl. apply c2. apply IHsubseq.
  - simpl. apply c3. apply IHsubseq.
Qed.
  (* FILL IN HERE *)
...
Proof.
  intros.
  induction ss as [|h t IH1].
  - simpl. apply MStar0.
  - simpl. simpl in H. apply MStarApp.
    + apply H. left. reflexivity.
    + apply IH1. intros. apply H. right. apply H0.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof.
  ...
  - discriminate Heqre'.
  - discriminate Heqre'.
  - injection Heqre' as . exists []. simpl. split.
    + reflexivity.
    + intros. destruct H0.
  - inversion Heqre' as [Heqre].  apply IH2 in Heqre'. destruct Heqre' as [ss [Hs2 H]]. exists (s1 :: ss). split.
    + simpl. rewrite Hs2. reflexivity.
    + intros. simpl in H0. destruct H0.
      -- rewrite <- H0. rewrite <- Heqre. apply Hmatch1.
...
Proof.
  ...
  - intros. simpl in H. apply Sn_le_Sm__n_le_m in H. inversion H.
  - intros. simpl in H. rewrite app_length in H. apply add_le_cases in H. destruct H.
    + apply IH1 in H. destruct H. destruct H. destruct H. exists x. exists x0. exists (x1 ++ s2). destruct H. rewrite -> H. split.
    -- rewrite <- app_assoc. rewrite <- app_assoc. reflexivity.
    -- destruct H0. split.
      ++ apply H0.
      ++ intros. assert(AppH1: x ++ napp m x0 ++ x1 ++ s2 = (x ++ napp m x0 ++ x1) ++ s2). {
      rewrite <- app_assoc. rewrite <- app_assoc. reflexivity.
      } rewrite AppH1.  apply MApp. apply H1. apply Hmatch2.
    + apply IH2 in H. destruct H. destruct H. destruct H. exists (s1 ++ x). exists x0. exists x1. destruct H. rewrite -> H. split.
    -- rewrite <- app_assoc. reflexivity.
    -- destruct H0. split.
      ++ apply H0.
      ++ intros. assert(AppH2: (s1 ++ x) ++ napp m x0 ++ x1 = s1 ++ (x ++ napp m x0 ++ x1)). {
      rewrite <- app_assoc. reflexivity.
      } rewrite  AppH2.  apply MApp. apply Hmatch1. apply H1.
  - intros. simpl in H. assert (MH1: pumping_constant re1 <= length s1). {
    apply le_trans with (n:= pumping_constant re1 + pumping_constant re2).
    - apply le_plus_l.
    - apply H.
  }
    apply IH in MH1. destruct MH1. destruct H0. destruct H0.
    destruct H0. destruct H1. exists x. exists x0. exists x1. split. 
    -- apply H0.
    -- split. apply H1. intros. apply MUnionL. apply H2.
  - intros. simpl in H. assert (MH1: pumping_constant re2 <= length s2). {
    apply le_trans with (n:= pumping_constant re1 + pumping_constant re2).
  ...
  - intros. simpl in H. inversion H. apply pumping_constant_0_false in H1. destruct H1.
  - simpl. intros H. destruct (lt_ge_cases (length s1) (pumping_constant re)) as  [Hgt1|H1].
    -- destruct s1 as [|x1 s1'].
      --- simpl. destruct (IH2 H) as [s21 [s22 [s23 [Heq2 [Hnonempty22 ]]]]].
          exists s21. exists s22. exists s23. split.
          ---- apply Heq2.
          ---- split.
                ----- apply Hnonempty22.
                ----- apply H0.
      --- exists []. exists (x1 :: s1'). exists s2. split.
          ---- reflexivity.
          ---- split.
                ----- discriminate.
                ----- intros m. apply napp_star.
                            ------- apply Hmatch1.
                            ------- apply Hmatch2.
    -- destruct (IH1 H1) as [s11 [s12 [s13 [Heq1 [Hnonempty12 Hmatch1m]]]]].
      exists s11. exists s12. exists (s13 ++ s2). split.
      --- rewrite Heq1. rewrite <- app_assoc. rewrite <- app_assoc. reflexivity.
      --- split.
          ---- apply Hnonempty12.
          ---- intros m. rewrite (app_assoc _ _ s13 s2). rewrite (app_assoc _ s11 _ s2). apply MStarApp.
                      ------ apply Hmatch1m.
                      ------ apply Hmatch2.
Qed.
...
Proof.
  ...
  (* FILL IN HERE *)
  - intros. simpl in H. apply Sn_le_Sm__n_le_m in H. inversion H.
  - simpl. intros H. destruct (lt_ge_cases (length s1) (pumping_constant re1)) as [Hgt1|H1].
  (* (* use pumping_constant re1 <= length s1 or pumping_constant re1 > length s1 *) *)
  (* rewrite app_length in H. destruct (add_le_cases _ _ _ _ H) as [H1|H2]. *)
  -- assert (H2 : pumping_constant re2 <= length s2).
      { rewrite app_length in H. destruct (add_le_cases _ _ _ _ H) as [H1|H2].
        - apply (le_trans (S (length s1)) (pumping_constant re1) (length s1)) in Hgt1.
          -- destruct (PeanoNat.Nat.nle_succ_diag_l _ Hgt1).
          -- apply H1.
        - apply H2. }
      destruct (IH2 H2) as [s21 [s22 [s23 [Heq2 [Hnonempty22 [Hlength2 Hmatch2m]]]]]].
      exists (s1 ++ s21). exists s22. exists s23. split.
      --- rewrite <- (app_assoc _ s1 s21 _). f_equal. apply Heq2.
      --- split.
          ---- apply Hnonempty22.
          ---- split.
                ----- apply le_trans with (length s1 + pumping_constant re2).
                      ------ rewrite app_length. rewrite <- add_assoc. apply plus_le_compat_l. apply Hlength2.
                      ------ apply plus_le_compat_r. apply Sn_le_Sm__n_le_m. apply le_S. apply Hgt1.
                ----- intros m. rewrite <- (app_assoc _ s1 s21 _).
                      apply MApp.
                      ------ apply Hmatch1.
                      ------ apply Hmatch2m.
    -- destruct (IH1 H1) as [s11 [s12 [s13 [Heq1 [Hnonempty12 [Hlength1 Hmatch1m]]]]]].
      exists s11. exists s12. exists (s13 ++ s2). split.
      --- rewrite (app_assoc _ s12 s13 s2). rewrite (app_assoc _ s11 _ s2). f_equal. apply Heq1.
      --- split.
          ---- apply Hnonempty12.
          ---- split.
                ----- apply le_trans with (pumping_constant re1).
                      ------ apply Hlength1.
                      ------ apply le_plus_l.
                ----- intros m. rewrite (app_assoc _ _ s13 s2). rewrite (app_assoc _ s11 _ s2).
                      apply MApp.
                      ------ apply Hmatch1m.
                      ------ apply Hmatch2.
  - intros. simpl in H. assert (MH1: pumping_constant re1 <= length s1). {
    ...
    apply IH in MH1. destruct MH1. destruct H0. destruct H0.
    destruct H0. destruct H1. destruct H2. exists x. exists x0. exists x1. split. 
    -- apply H0.
  ...
  - simpl. intros H. destruct (lt_ge_cases (length s1) (pumping_constant re)) as [Hgt1|H1].
    -- destruct s1 as [|x1 s1'].
      --- simpl. destruct (IH2 H) as [s21 [s22 [s23 [Heq2 [Hnonempty22 [Hlength2 Hmatch2m]]]]]].
          exists s21. exists s22. exists s23. split.
          ---- apply Heq2.
          ---- split.
                ----- apply Hnonempty22.
      ...
      --- exists []. exists (x1 :: s1'). exists s2. split.
          ...
          ---- split.
                ----- discriminate.
                ----- split.
                      ------ apply Sn_le_Sm__n_le_m. apply le_S. apply Hgt1.
                      ------ intros m. apply napp_star.
                            ------- apply Hmatch1.
                            ------- apply Hmatch2.
    -- destruct (IH1 H1) as [s11 [s12 [s13 [Heq1 [Hnonempty12 [Hlength1 Hmatch1m]]]]]].
...
Proof.
  ...
  - intros. destruct H.
    -- reflexivity.
    -- unfold not in H. apply H in H0. destruct H0.
  - intros. destruct H.
    -- apply H.
    -- discriminate.
Qed.
...
Proof.
  ...
  - simpl. simpl in Hcount. destruct (eqbP n m).
    -- discriminate Hcount.
    -- simpl in Hcount. apply IHl' in Hcount. unfold not. intros. destruct H0.
      + rewrite H0 in H. destruct H. reflexivity.
      + unfold not in Hcount. apply Hcount in H0. apply H0.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
| nostutterOne (x : X): nostutter [x]
| nostutterApp (x y : X)(l: list X): x <> y -> nostutter (y :: l) -> nostutter (x :: y :: l)
 (* FILL IN HERE *)
.
...
Proof.
  apply nostutterApp. discriminate.
  apply nostutterApp. discriminate.
  apply nostutterApp. discriminate.
  apply nostutterApp. discriminate.
  apply nostutterApp. discriminate.
...
| Mergenil : merge [] [] []
| MergeL (l1 l2 l :list X) (x : X) : merge l1 l2 l -> merge (x::l1) l2 (x::l)
| MergeR (l1 l2 l :list X) (x : X) : merge l1 l2 l -> merge l1 (x::l2) (x::l)
.
...
Proof.
  ...
  induction H.
  - simpl. reflexivity.
  - simpl. simpl in H0. destruct H0. rewrite H0. apply f_equal. apply IHmerge.
    + apply H2. + apply H1.
  - simpl. simpl in H1. destruct H1. rewrite H1. apply IHmerge.
    + apply H0. + apply H2.
Qed.
...
Proof.
  ...
  - simpl. assert (H1: h :: t ++ rev t ++ [h] = h :: (t ++ rev t) ++ [h]). {
    rewrite app_assoc. reflexivity.
  } rewrite H1. apply palx with (x:=h). apply IHl'.
Qed.
...
Lemma palindrome_converse_length: forall n {X: Type} (l: list X),
    length l <= n -> l = rev l -> pal l.
Proof.
  induction n as [|n' IHn'].
  - (* n = 0 *)
    intros X [|x l'] Hlen Hrev.
    -- apply palEmpty.
    -- inversion Hlen.
  - (* n = S n' *)
    intros X [|x l'] Hlen Hrev.
    -- apply palEmpty.
    -- simpl in Hrev. destruct (rev l') as [|x' revl'] eqn:Erevl'.
        --- inversion Hrev as [El']. apply palsingle.
        --- inversion Hrev as [[Ex El']]. apply palx. apply IHn'.
            ---- apply Sn_le_Sm__n_le_m. apply Sn_le_Sm__n_le_m. apply le_S.
                  replace (S (length revl')) with (length revl' + length [x']).
                  ----- rewrite <- app_length. rewrite <- El'. apply Hlen.
                  ----- rewrite add_comm. reflexivity.
            ---- rewrite El' in Erevl'. rewrite rev_app_distr in Erevl'.
                  injection Erevl' as Erevl''. symmetry. apply Erevl''.
Qed.

Theorem palindrome_converse: forall {X: Type} (l: list X),
    l = rev l -> pal l.
Proof.
  intros X l H. apply palindrome_converse_length with (length l).
  - apply le_n.
  - apply H.
Qed.
...
Proof.
  ...
  - simpl in H. destruct H.
    ...
    + apply IHl in H. destruct H. destruct H. rewrite H. exists (x0::x1). exists x2.
    simpl. reflexivity.
Qed.
...
| repeats_here x l : In x l -> repeats (x :: l)
| repeats_there x l : repeats l -> repeats (x :: l)
.
...
Proof.
  ...
  - intros l2 Hin12 Hlength12. inversion Hlength12.
  - intros l2 Hin12 Hlength12. destruct (EM (In x l1')) as [Hin1| Hnotin1].
    -- apply repeats_here. apply Hin1.
    -- assert (Hin2 : In x l2).
      { apply Hin12. left. reflexivity. }
      destruct (in_split _ _ _ Hin2) as [l21 [l22 Heq2]].
      apply repeats_there. apply IHl1' with (l21 ++ l22).
      --- intros x' Hin1'.
          assert (Hin2' : In x' l2).
          { apply Hin12. right. apply Hin1'. }
          rewrite Heq2 in Hin2'. apply In_app_iff in Hin2'. apply In_app_iff. destruct Hin2' as [Hin21'|Hin22'].
          ---- left. apply Hin21'.
          ---- destruct Hin22' as [contra|Hin22'].
                ----- rewrite contra in Hnotin1. destruct (Hnotin1 Hin1').
                ----- right. apply Hin22'.
      --- rewrite Heq2 in Hlength12. rewrite app_length in Hlength12.
          rewrite app_length. apply Sn_le_Sm__n_le_m. rewrite plus_n_Sm. apply Hlength12.
Qed.
...
Proof.
  intros.
  split.
  - intros. apply app_exists in H. destruct H. destruct H. destruct H. destruct H0. destruct x as [|h t]. 
    + simpl in H. left. split. apply H0. rewrite H. apply H1.
    + simpl in H. right. inversion H. exists t. exists x0. split. reflexivity.
      split. apply H0. apply H1.
  - intros. destruct H.
    + destruct H. apply MApp with (s1:=[]). apply H. apply H0.
    + destruct H. destruct H. destruct H. destruct H0.
      -- rewrite H. apply MApp with (s1:=a::x). apply H0. apply H1.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof.
  intros a s re. split.
  - intros Hmatch. remember (a :: s) as s'. remember (Star re) as re'.
    induction Hmatch as [| | | | | |s0' s1' re' Hmatch0' IH0' Hmatch1' IH1'].
    -- (* Hmatch = MEmpty *)
      discriminate.
    -- (* Hmatch = MChar ... *)
      discriminate.
    ...
    -- (* Hmatch = MStar0 ... *)
      discriminate.
    -- (* Hmatch = MStarApp ... *)
      destruct s0' as [|x0 s0''].
      --- destruct (IH1' Heqs' Heqre') as [s10 [s11 [Es1 [Hmatch10 Hmatch11]]]].
          exists s10. exists s11. split.
          ---- apply Es1.
          ---- split.
                ----- apply Hmatch10.
                ----- apply Hmatch11.
      --- inversion Heqre' as [Ere]. inversion Heqs' as [[Ea Es]]. exists s0''. exists s1'. split.
          ---- reflexivity.
          ---- rewrite <- Ere. split.
                ----- rewrite <- Ea. apply Hmatch0'.
                ----- apply Hmatch1'.
  - intros [s0 [s1 [Es [Hmatch0 Hmatch1]]]]. rewrite Es. apply (MStarApp (a :: s0)).
    -- apply Hmatch0.
    -- apply Hmatch1.
Qed.
